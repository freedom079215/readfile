...
var GardenUtils = {
    plugin: {
		...
		screenMoveToEle: function(obj) {
            ...
            iOS = false;
            if(iOS){
				GardenUtils.plugin.screenMoving = true;
                $("html, body").animate({
                    'scrollTop': $(obj.moveToObj).position().top-obj.minHeight
                }, 1000,function(){
					setTimeout(function(){
						GardenUtils.plugin.screenMoving = false;
					},100);
				});
            }
            else{
                ...
                if (datas.length <= options.items) {
                    ...
                    if (!options.page.isDisable) {
                        ...
                        $(target).find('.owl-dot').each(function(i) {
                            ...
                            if ( datas[i].hasOwnProperty('title') || options.page.type == 'text') {
        ...
        fontSize : function(conf) {
            ...
            if(target != undefined && target.find('.textlv-area').length == 0) {
                var textlvArea = $('<div class="textlv-area"><ul><li><button type="button" class="btn btn-default btn-font-small fontsize">'+sWord+'</button></li><li><button type="button" class="btn btn-default btn-font-mid fontsize active">'+mWord+'</button></li><li><button type="button" class="btn btn-default btn-font-big fontsize">'+lWord+'</button></li></ul></div>');
                
                textlvArea.find('button.btn-font-big').click(function(ev){
                    ev.preventDefault();
                    textlvArea.find('button.fontsize').removeClass('active');
                    _fontSize(target,'17','30');
    ...
    display: {
        ...
        tab: function(conf) {
            ...
            $(target).find('.ga-tab-title-container').each(function(){
                var container = $(this);
                var max = container.attr('ga-tab-max');

                if (typeof max === typeof undefined || max === false) {
                    max = 6;
                }

                var tabTitle_ele = container.children('li');
                if( (typeof max !== typeof undefined && max !== false)
                   && tabTitle_ele.length > max){
                    tabTitle_ele.each(function(i){
                        if( (i+1) > max ){
                            $(this).remove();
                        }
                    });
            ...
            }); // end each: ga-tab-title-container[ga-tab-overflow="arrow"]
            ...
            $(target).find('.ga-tab-title-container[ga-tab-overflow="dropdown"]').each(function() {
                var container = $(this);
                var max = container.attr('ga-tab-overflow-max');
                ...
                if (container.children('li').length > max) {
                    ...
                    var dropdown_container = $('<div class="ga-tab-dropdown">'
                        +'<div class="ga-tab-dropdown-content dropdown">'
                            +'<a href="#" class="dropdown-toggle ga-tab-dropdown-btn"'
                            +' data-toggle="dropdown"></a>'
                            +'<ul class="ga-tab-dropdown-ul dropdown-menu"></ul>'
                        +'</div></div>').insertAfter(container);

                    var dropdown_ul = dropdown_container.find('.ga-tab-dropdown-ul').first();
                    container.children('li').each(function(){
                        var li_ele = $(this);
                        var a_ele = li_ele.find('a').first();

                        dropdown_ul.append('<li class="ga-tab-dropdown-li">'
                                +'<a href="'+a_ele.attr('href')+'" ga-tab="dropdown">'+li_ele.text()+'</a>'
                            +'</li>');
                    }); // end each: li
                }
            }); // end each: ga-tab-title-container[ga-tab-overflow="dropdown"]
            ...
            }); // end click: [ga-tab="tab"]

            $(target).find('[ga-tab="dropdown"]').off('click').on('click', function(ev) {
                ev.preventDefault();

                var dropdown_aEle = $(this);
                var dropdown_container = dropdown_aEle.parents('.ga-tab-dropdown').first();
                var dropdown_btn = dropdown_container.find('.ga-tab-dropdown-btn').first();
                dropdown_btn.text( dropdown_aEle.text() );

                var href = dropdown_aEle.attr('href');
                var content_container = $(href).parents('.ga-tab-content-container').first();
                var title_container = content_container.parents('.ga-tab-container').first().children('.ga-tab-title-container');
                ...
                dropdown_aEle.removeClass('tab-click');
            ...
            var tab_resize_trigger = function() {
                ...
                $(target).children('.ga-tab-title-container').each(function() {
                    $(this).find('li > [ga-tab]').parent().each(function(i) {
                ...
                if (window_w > 768 && !hasActive || tabId != '') {
                    if (tabId != '') {
                        ...
                        $(target).find('[ga-tab="collapse"], [ga-tab="dropdown"]').first().trigger('click');
                    }
                } else if( hasActive ){
                    $(target).find('[ga-tab="dropdown"]').eq(active_ele_i).trigger('click');
                } else {
                    // fixFooter();
                }

                $('[ga-tab-overflow="dropdown"]').each(function(){
                    var container = $(this);
                    var parent_ele = container.parents('.ga-tab-container').first();
                    var max = container.attr('ga-tab-overflow-max');
                    ...
                    var dropdown_container = parent_ele.find('.ga-tab-dropdown').first();
                    if( dropdown_container.length > 0 ){
                        if( window_w > 768 ){
                            dropdown_container.addClass('hidden');
                            container.removeClass('hidden');
                        } else {
                            dropdown_container.removeClass('hidden');
                            container.addClass('hidden');
                        }
                    }
                }); // end each: ga-tab-overflow="dropdown"
